...
def initialize_files():
    ...
    try:
        with open(DAILY_CHALLENGE_FILE, 'r') as f:
            pass
    except FileNotFoundError:
        with open(DAILY_CHALLENGE_FILE, 'w') as f:
            json.dump({"last_challenge_date": None, "questions": []}, f)
...
def register_user():
    ...
    users[username] = {"password": password, "scores": [], "achievements": [], "hints_used": 0}
    with open(USER_DATA_FILE, 'w') as f:
        json.dump(users, f)
...
def generate_daily_challenge():
    ...
    if daily_data["last_challenge_date"] == today:
        return daily_data["questions"]
    ...
    daily_data = {"last_challenge_date": today, "questions": daily_questions}
    with open(DAILY_CHALLENGE_FILE, 'w') as f:
...
def display_leaderboard():
    try:
        ...
        print("Leaderboard is empty. Start playing to top the leaderboard!")
...
def display_achievements(username):
    try:
        with open(USER_DATA_FILE, 'r') as f:
            users = json.load(f)
        achievements = users[username].get("achievements", [])
        if achievements:
            print("\n🏆 Your Achievements:")
        ...
        print("User data file is missing. Please check the system.")
...
def main_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            selected_questions = questions[category][difficulty][:10]  
